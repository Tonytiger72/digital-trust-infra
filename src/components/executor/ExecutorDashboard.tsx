  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard icon={<FileCheck className="w-5 h-5 text-blue-600" />} label="Proyectos activos" value={2} />
        <StatCard icon={<Clock className="w-5 h-5 text-amber-600" />} label="Pendiente validación" value={pending} />
        <StatCard icon={<CheckCircle2 className="w-5 h-5 text-green-600" />} label="Hitos validados" value={validated} />
        <StatCard icon={<AlertCircle className="w-5 h-5 text-slate-500" />} label="Total hitos" value={totalMilestones} />
      </div>
...
function ProjectCard({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-white rounded-lg border border-slate-200 p-4 text-left hover:border-[#2E75B6] hover:shadow-sm transition-all group"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <ProgressBar value={project.completionPercent} showLabel size="sm" />
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-slate-500">
            ...
            <span className="text-slate-400">Ref: {project.contractRef}</span>
          </div>
        </div>
